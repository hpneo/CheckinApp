...
namespace CheckinAppAndroid
{
	...
	public class DeleteCatalogDialogFragment : DialogFragment
	{
		...
		public override Dialog OnCreateDialog (Bundle bundle)
		{
			...
			builder.SetNegativeButton ("Sí", delegate (object sender, DialogClickEventArgs e) {
				...
				if (catalogs.Delete(Catalog)) {
					...
					((ListCatalogActivity) Activity).Adapter.Remove(Catalog);
					((ListCatalogActivity) Activity).Adapter.NotifyDataSetChanged();

					Toast.MakeText(Activity, "Catálogo eliminado", ToastLength.Long).Show();
				}
			});
			builder.SetPositiveButton ("No", delegate (object sender, DialogClickEventArgs e) {
			});
